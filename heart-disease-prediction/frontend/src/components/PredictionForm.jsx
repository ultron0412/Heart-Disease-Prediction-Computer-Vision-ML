...
const PredictionForm = ({ onPrediction, onError, onLoadingChange, isLoading }) => {
  ...
  const handleInputChange = (e) => {
    ...
    const numValue = ['age', 'trestbps', 'chol', 'thalach'].includes(name) 
      ? parseInt(value) 
      : name === 'oldpeak' 
      ? parseFloat(value)
      : name === 'sex' || name === 'cp' || name === 'fbs' || name === 'restecg' || name === 'exang' || name === 'slope' || name === 'ca' || name === 'thal'
      ? parseInt(value)
      : value
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add clinical data as JSON string
      ...
      const errorMessage = error.response?.data?.detail || 
                          error.message || 
                          'Failed to get prediction. Make sure the backend is running on http://localhost:8000'
      ...
      <div className="image-section">
        ...
        <div className="image-upload">
          <input
            ...
            disabled={isLoading}
          />
          <label htmlFor="ecg-image" className="upload-label">
            {ecgImage ? '✓ Image selected' : '📤 Choose ECG Image'}
          </label>
        </div>
